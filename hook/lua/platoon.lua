...
Platoon = Class(RNGAIPlatoon) {
    ...
    EngineerBuildAIRNG = function(self)
        ...
        elseif cons.AvoidCategory then
            ...
            reference  = AIUtils.FindUnclutteredAreaRNG(aiBrain, cat, pos, radius, cons.maxUnits, cons.maxRadius, avoidCat)
            buildFunction = AIBuildStructures.AIBuildAdjacencyRNG
    ...
    HuntAIRNGHero = function(self)
        ...
        while PlatoonExists(aiBrain, self) do
            ...
            if target then
                ...
                if EntityCategoryContains(categories.COMMAND, target) and not friendlyCdrThreat>0 then
                ...
                if attackUnits then
                    ...
                    local searchpos = RUtils.lerpy(platoonPos,targetPosition,{returnDist,5+Hero.MaxWeaponRange})
    ...
    HuntAIRNGHeroSniper = function(self)
        self:Stop()
        local aiBrain = self:GetBrain()
        local armyIndex = aiBrain:GetArmyIndex()
        local target
        ...
        while PlatoonExists(aiBrain, self) do
            ...
            if target then
                ...
                local targetDist = VDist2(platoonPos[1],platoonPos[3],targetPosition[1],targetPosition[3])
                friendlyThreat = aiBrain:GetThreatAtPosition(platoonPos, 1, true, 'AntiSurface', aiBrain:GetArmyIndex())
                local enemyThreat = aiBrain:GetThreatAtPosition(targetPosition, 1, true, 'Land')
                local enemyCdrThreat = aiBrain:GetThreatAtPosition(targetPosition, 1, true, 'Commander')
                local friendlyCdrThreat = aiBrain:GetThreatAtPosition(platoonPos, 1, true, 'Commander', aiBrain:GetArmyIndex())
                if EntityCategoryContains(categories.COMMAND, target) and not friendlyCdrThreat>0  then
                    if platoonThreat + friendlyThreat < 30 then
                        ...
                        while platoonThreat + friendlyThreat + friendlyCdrThreat > enemyThreat and not target.Dead do
                            Hero:SetCustomName('Hero Movement: Engaging Comm Harrass')
                            platoonThreat = self:CalculatePlatoonThreat('AntiSurface', categories.ALLUNITS)
                            friendlyThreat = aiBrain:GetThreatAtPosition(platoonPos, 1, true, 'AntiSurface', aiBrain:GetArmyIndex())
                            enemyThreat = aiBrain:GetThreatAtPosition(targetPosition, 1, true, 'Land')
                            friendlyCdrThreat = aiBrain:GetThreatAtPosition(platoonPos, 1, true, 'Commander', aiBrain:GetArmyIndex())
                            platoonPos = Hero:GetPosition()
                            targetPosition = target:GetPosition()
                            targetDist=VDist2(platoonPos[1],platoonPos[3],targetPosition[1],targetPosition[3])
                            if targetDist<Hero.MaxWeaponRange*0.9 then
                                local returnPos = AIUtils.RandomLocation(homepos[1],homepos[3])
                                LOG('hero attempt move returnhome')
                                Hero:SetCustomName('Hero Movement: Too Close! Sniper Panic')
                                IssueClearCommands({Hero})
                                IssueMove({Hero},returnPos)
                                WaitTicks(60)
                            end
                            local kitePos = RUtils.lerpy(targetPosition, platoonPos, {targetDist, Hero.MaxWeaponRange})
                            self:MoveToLocation(kitePos, false, 'Attack')
                            WaitTicks(10)
                            IssueAttack({Hero}, target)
                            WaitTicks(10)
                        end
                ...
                if attackUnits then
                    ...
                    local testpos = RUtils.lerpy(platoonPos,position,{returnDist,5+Hero.MaxWeaponRange})
                    local searchpos = RUtils.lerpy(platoonPos,targetPosition,{returnDist,5+Hero.MaxWeaponRange})
                    if returnDist>Hero.MaxWeaponRange then
                        self:MoveToLocation(searchpos, false, 'Attack')
                    end
                    friendlyThreat = aiBrain:GetThreatAtPosition(platoonPos, 2, true, 'AntiSurface', aiBrain:GetArmyIndex())
                    enemyThreat = aiBrain:GetThreatAtPosition(testpos, 1, true, 'AntiSurface')
                    ...
                    while PlatoonExists(aiBrain, self) do
                        if target and not target.Dead then
                            ...
                            Hero.friendlythreat=friendlyThreat
                                ...
                                if targetDist<Hero.MaxWeaponRange*0.7 then
                                    local returnPos = AIUtils.RandomLocation(homepos[1],homepos[3])
                                    LOG('hero attempt move returnhome')
                                    Hero:SetCustomName('Hero Movement: Too Close! Sniper Panic')
                                    IssueClearCommands({Hero})
                                ...
                                if enemyThreat > (platoonThreat/5 + 1.5*friendlyThreat + friendlyCdrThreat*1.5) then
                                    local returnPos = AIUtils.RandomLocation(homepos[1],homepos[3])
                                    LOG('hero attempt move returnhome')
                                    Hero:SetCustomName('Hero Movement: HighThreat')
                                    IssueClearCommands({Hero})
                                ...
                                if (enemyCdrThreat + enemyThreat) > (platoonThreat/5 + 1.5*friendlyThreat + friendlyCdrThreat) then
                                    local returnPos = AIUtils.RandomLocation(homepos[1],homepos[3])
                                    LOG('hero attempt move returnhome')
                                    Hero:SetCustomName('Hero Movement: Cdr HighThreat')
                                    IssueClearCommands({Hero})
                                ...
                                -- in case we don't move, check if we can fire at the target
                                    ...
                                    if aiBrain:CheckBlockingTerrain(platoonPos, targetPosition, Hero.WeaponArc) then
                                        ...
                                        local returnPos = AIUtils.RandomLocation(homepos[1],homepos[3])
                                        LOG('hero attempt move returnhome')
                                        Hero:SetCustomName('Hero Movement: Weapon Blocked- returning')
                                        IssueMove({Hero}, returnPos )
                                        WaitTicks(10)
                                    else
                                        --unit:SetCustomName('Fight micro SHOOTING ['..repr(target.UnitId)..'] dist: '..dist)
    ...
    TrueHeroRNG = function(self)
        ...
        Hero.home=homepos
        Hero.base=homepos
        Hero.evaluationpoints = {}
        Hero.friendlyThreats = {}
        Hero.enemyThreats = {}
        Hero.threats = {}
        Hero.Pos=Hero:GetPosition()
        LOG('Hero homebase: '..repr(homepos)..' startpos = '..repr({homebasex,homebasey}))
        if not Hero.Dead then
            for _, weapon in Hero:GetBlueprint().Weapon or {} do
                if not Hero.MaxWeaponRange or weapon.MaxRadius > Hero.MaxWeaponRange then
                    Hero.MaxWeaponRange = weapon.MaxRadius * 0.9
                    if weapon.BallisticArc == 'RULEUBA_LowArc' then
                        Hero.WeaponArc = 'low'
                    elseif weapon.BallisticArc == 'RULEUBA_HighArc' then
                        Hero.WeaponArc = 'high'
            ...
            Hero.evaluationradius=Hero.MaxWeaponRange*0.7
            LOG('hero evaluationradius = '..repr(Hero.evaluationradius))
        end
        while PlatoonExists(aiBrain, self) do
            ...
            if closestmex then
                --[[if not Hero:CanPathTo(alliedmexes[1]:GetPosition()) then
                    Hero.home=Hero.base
                else]]
            ...
            for i,v in Hero.evaluationpoints do
                ...
                friendlyThreat=friendlyThreat+Hero.friendlyThreats[i]
                enemyThreat=enemyThreat+Hero.enemyThreats[i]
            end
            Hero.ThreatLimit=(friendlyThreat/2+platoonThreat)*health
            if enemyThreat>Hero.ThreatLimit then
                --ENGAGE RUNAWAYMODE
                local runawaydirection = 0
                local evalweight=0
                for i,v in Hero.evaluationpoints do
                    if Hero.threats[i]<0 then
                        runawaydirection=runawaydirection-i*math.pi/4*Hero.threats[i]
                        evalweight=evalweight-Hero.threats[i]
                    else
                        runawaydirection=runawaydirection+(i*math.pi/4+math.pi)*Hero.threats[i]
                        evalweight=evalweight+Hero.threats[i]
                    end
                end
                local angle=runawaydirection/evalweight
                local runawaypoint={Hero.Pos[1]+math.cos(angle)*Hero.MaxWeaponRange,Hero.Pos[2],Hero.Pos[3]+math.sin(angle)*Hero.MaxWeaponRange}
                Hero.dest=runawaypoint
                IssueClearCommands({Hero})
                IssueMove({Hero},runawaypoint)
                WaitTicks(30)
                Hero.dest={Hero.home[1]+math.random(-4,4),Hero.home[2],Hero.home[3]+math.random(-4,4)}
                IssueClearCommands({Hero})
                IssueMove({Hero},Hero.home)
                WaitTicks(30)
                continue
            end
            Hero.target=nil
            ...
            if targetacu then
                Hero.targetacu=targetacu:GetPosition()
                targetacuDist=VDist2(Hero.targetacu[1],Hero.targetacu[3],Hero.Pos[1],Hero.Pos[3])
                if targetacuDist>150 then
                    Hero.targetacu=nil
                end
            end
            if targetmex then
                Hero.targetmex=targetmex:GetPosition()
                targetmexDist=VDist2(Hero.targetmex[1],Hero.targetmex[3],Hero.Pos[1],Hero.Pos[3])
                if targetmexDist>150 then
                    Hero.targetmex=nil
                end
            end
            if targeteng then
                Hero.targeteng=targeteng:GetPosition()
                targetengDist=VDist2(Hero.targeteng[1],Hero.targeteng[3],Hero.Pos[1],Hero.Pos[3])
                if targetengDist>150 then
                    Hero.targeteng=nil
                end
            end
            if targetpd then
                Hero.targetpd=targetpd:GetPosition()
                targetpdDist=VDist2(Hero.targetpd[1],Hero.targetpd[3],Hero.Pos[1],Hero.Pos[3])
            ...
            if not target or target.Dead --[[or not Hero:CanPathTo(targetPosition)]] then
                Hero.target=nil
            ...
            elseif health<0.5 and Hero.Sync.regen>0 then
                ...
                Hero.target=targetPosition
                targetDist=VDist2(targetPosition[1],targetPosition[3],Hero.Pos[1],Hero.Pos[3])
                Hero.dest = RUtils.lerpy({Hero.Pos[1],Hero.Pos[2],Hero.Pos[3]},targetPosition,{targetDist,targetDist - 3 * Hero.MaxWeaponRange})
                IssueClearCommands({Hero})
                IssueMove({Hero},Hero.dest)
                WaitTicks(40)
                Hero.dest=Hero.home
            ...
            elseif targetpd and targetpdDist<Hero.MaxWeaponRange*2 and not Hero.MaxWeaponRange>29 then
                ...
                IssueMove({Hero},Hero.dest)
                WaitTicks(30)
            elseif targetpd and targetpdDist<targetDist*0.9 and Hero.MaxWeaponRange>29 and targetpdDist<Hero.MaxWeaponRange*1.3 then
                Hero.dest = RUtils.lerpy({Hero.Pos[1],Hero.Pos[2],Hero.Pos[3]},Hero.targetpd,{targetpdDist,targetpdDist - Hero.MaxWeaponRange})
                target=targetpd
                local targetPosition=target:GetPosition()
                Hero.target=targetPosition
                IssueClearCommands({Hero})
            ...
            elseif targetacu and targetacuDist<Hero.MaxWeaponRange*2 and not Hero.MaxWeaponRange>29 then
                ...
                IssueMove({Hero},Hero.dest)
                WaitTicks(30)
            elseif targetacu and targetacuDist<Hero.MaxWeaponRange*2 and Hero.MaxWeaponRange>29 and targetacuDist<targetDist*0.9 then
                Hero.dest = RUtils.lerpy({Hero.Pos[1],Hero.Pos[2],Hero.Pos[3]},Hero.targetacu,{targetacuDist,targetacuDist - Hero.MaxWeaponRange})
                target=targetacu
                local targetPosition=target:GetPosition()
                Hero.target=targetPosition
                IssueClearCommands({Hero})
            ...
            elseif target and targetDist>160 and not EntityCategoryContains(categories.ENGINEER + categories.STRUCTURE,target) then
                ...
                if targeteng and targetengDist<targetDist*1.5 then
                    IssueClearCommands({Hero})
                    IssueAttack({Hero},targeteng)
                    WaitTicks(40)
                elseif targetmex and targetmexDist<targetDist*1.5 then
                    IssueClearCommands({Hero})
                    IssueAttack({Hero},targetmex)
                    WaitTicks(40)
                end
                if not target or target.Dead then continue end
                if enemyThreat<Hero.ThreatLimit/2 and not Hero.Sniper and not targetacuDist<40 or enemyThreat<Hero.ThreatLimit/5 then
                    local targetPosition=target:GetPosition()
                    Hero.target=targetPosition
                    targetDist=VDist2(targetPosition[1],targetPosition[3],Hero.Pos[1],Hero.Pos[3])
                    ...
                    Hero.target=targetPosition
                    targetDist=VDist2(targetPosition[1],targetPosition[3],Hero.Pos[1],Hero.Pos[3])
                    if targetDist<Hero.MaxWeaponRange*2.5 then
                        local smartPos = RUtils.lerpy({Hero.Pos[1]+math.random(-2,2),Hero.Pos[2],Hero.Pos[3]+math.random(-2,2)},targetPosition,{targetDist,targetDist - Hero.MaxWeaponRange})
                        smartPos = {smartPos[1]+math.random(-1,1),smartPos[2],smartPos[3]+math.random(-1,1)}
                        local strafeshift=RUtils.LerpyRotate(Hero.Pos,smartPos,{4,math.random(-4,4)})
                        Hero.dest=strafeshift
                        IssueClearCommands({Hero})
    ...
    HighlightTrueHero = function(self)
        ...
        while not Hero.dead and PlatoonExists(aiBrain, self) do
                ...
                pos1=Hero.pos
                pos2=Hero.dest
                DrawLinePop(pos1,Hero.home,'3f00FF21')
                if not Hero.pos then
                    WaitTicks(2)
                    continue 
                end
                if Hero.target then
                    DrawLinePop(pos1,Hero.target,'8fFF1155')
                end
                if Hero.targetmex then
                    DrawLinePop(pos1,Hero.targetmex,'1f4CFF00')
                end
                if Hero.targetacu then
                    DrawLinePop(pos1,Hero.targetacu,'4f4800FF')
                end
                if Hero.targetpd then
                    DrawLinePop(pos1,Hero.targetpd,'5fFF6A00')
                end
                ...
                if Hero.friendlyThreats and Hero.enemyThreats then
                    for i,v in Hero.evaluationpoints do
                        DrawCircle(v,math.min(Hero.evaluationradius*Hero.enemyThreats[i]/Hero.ThreatLimit,Hero.evaluationradius*1.2),'13FF0000')
                        DrawCircle(v,math.min(Hero.evaluationradius*Hero.friendlyThreats[i]/Hero.ThreatLimit,Hero.evaluationradius*1.2),'130000FF')
                        DrawCircle(v,Hero.evaluationradius,'1a808080')
                    end
                end
                if Hero.MaxWeaponRange then
                    DrawCircle(pos1,Hero.MaxWeaponRange,'3a808080')
                end
            WaitTicks(2)
        end
    end,
    TrueBomberRNG = function(self)
        self:Stop()
        local aiBrain = self:GetBrain()
        local armyIndex = aiBrain:GetArmyIndex()
        local target
        ...
        while PlatoonExists(aiBrain, self) do
            local platoonPos=self:GetPlatoonPosition()
            local targetpos=nil
            local targetacupos=nil
            local targetmexpos=nil
            local targetengpos=nil
            --if not target or target.Dead then
                target = self:FindClosestUnit('Attack', 'Enemy', true, categories.ALLUNITS - categories.COMMAND - categories.AIR - categories.WALL)
            --end
            local targetacuDist
            targetacu = self:FindClosestUnit('Attack', 'Enemy', true, categories.COMMAND)
            local targetmexDist
            targetmex = self:FindClosestUnit('Attack', 'Enemy', true, categories.MASSEXTRACTION + categories.ENERGYPRODUCTION + categories.ENERGYSTORAGE)
            local targetengDist
            if not targeteng or targeteng.Dead then
                targeteng = self:FindClosestUnit('Attack', 'Enemy', true, categories.ENGINEER - categories.AIR - categories.NAVAL - categories.COMMAND)
            end
            if targetacu then
                targetacupos=targetacu:GetPosition()
                targetacuDist=VDist2(targetacupos[1],targetacupos[3],platoonPos[1],platoonPos[3])
            end
            if targetmex then
                targetmexpos=targetmex:GetPosition()
                targetmexDist=VDist2(targetmexpos[1],targetmexpos[3],platoonPos[1],platoonPos[3])
            end
            if targeteng then
                targetengpos=targeteng:GetPosition()
                targetengDist=VDist2(targetengpos[1],targetengpos[3],platoonPos[1],platoonPos[3])
            end
            local targetPosition
            local targetDist
            if target then
                ...
                targetDist=VDist2(targetpos[1],targetpos[3],platoonPos[1],platoonPos[3])
                if targetengDist<targetDist*1.5 then 
                    target=targeteng 
                    targetDist=targetengDist 
                elseif targetmexDist<targetDist*1.5 then
                    target=targetmex
                    targetDist=targetmexDist
                end
            end
            if not target or target.Dead --[[or not Hero:CanPathTo(targetPosition)]] then
                if VDist2(homebasex,homebasey,platoonPos[1],platoonPos[3])>50 then
                    for _,Hero in platoonUnits do
                        IssueClearCommands({Hero})
                        IssueMove({Hero},AIUtils.RandomLocation(homebasex,homebasey))
                    end
                end
                WaitTicks(100)
            elseif targetDist<3 then
                for _=0,20,1 do
                    if not target or target.Dead then break end
                    for _,Hero in platoonUnits do
                        targetpos=target:GetPosition()
                        IssueClearCommands({Hero})
                        IssueMove({Hero},targetpos)
                    end
                    WaitTicks(1)
            ...
            elseif targetDist<30 then
                ...
                if targeteng and targetengDist<targetDist*1.5 then
                    IssueClearCommands(platoonUnits)
                    IssueMove(platoonUnits,targetengpos)
                elseif targetmex and targetmexDist<targetDist*1.5 then
                    IssueClearCommands(platoonUnits)
                    IssueMove(platoonUnits,targetmexpos)
                end
                if not target or target.Dead then continue end
                targetpos=target:GetPosition()
                IssueClearCommands(platoonUnits)
                IssueMove(platoonUnits,targetpos)
                IssueAttack(platoonUnits,target)
                WaitTicks(5)
            end
            WaitTicks(2)
    ...
    TruePlatoonRNG = function(self)
        ...
        while PlatoonExists(aiBrain, self) do
            ...
            for _,v in platoonUnits do
                ...
                if VDist3Sq(v:GetPosition(),platoon.Pos)>v.MaxWeaponRange/5*v.MaxWeaponRange/5+platoonNum then
                    IssueClearCommands({v})
                    IssueMove({v},RUtils.LerpyRotate(v:GetPosition(),platoon.Pos,{VDist3(v:GetPosition(),platoon.Pos),v.MaxWeaponRange/6+math.sqrt(platoonNum)}))
            ...
            for _,v in platoonUnits do
                if not v or v.Dead then continue end
                platoon.health=platoon.health+v:GetHealthPercent()
            end
            platoon.health=platoon.health/platoonNum
            local alliedmexes=table.copy(aiBrain:GetListOfUnits(categories.MASSEXTRACTION + categories.ENGINEER, false, true))
            if alliedmexes[1] then
            ...
            if not target and not targetacu or targetDist>platoon.MaxWeaponRange*1.5 or (not target and targetacuDist>platoon.MaxWeaponRange*2) or not AIAttackUtils.CanGraphTo(AIAttackUtils.GetMostRestrictiveLayer(platoon),targetPosition,'Land') then
                ...
                if target then
                    platoon.path=AIAttackUtils.PlatoonGenerateSafePathToRNG(aiBrain, self.MovementLayer, platoon.Pos, targetPosition, -20, 150,ScenarioInfo.size[1]*ScenarioInfo.size[2])
                ...
                platoon.ThreatLimit=(platoon.friendlyThreat+platoonThreat)
                if platoon.enemyThreat>platoon.ThreatLimit then
                    ...
                    platoon.path=AIAttackUtils.PlatoonGenerateSafePathToRNG(aiBrain, self.MovementLayer, platoon.Pos, platoon.dest, 10, 150,ScenarioInfo.size[1]*ScenarioInfo.size[2])
                    if not platoon.path then 
                        local runawaydirection = 0
                        local evalweight=0
                        for i,v in platoon.evaluationpoints do
                ...
                if platoon.enemyThreat<platoon.ThreatLimit/2 and not platoon.Sniper and not targetacuDist<40 or platoon.enemyThreat<platoon.ThreatLimit/5 then
                    ...
                    if targetDist<platoon.MaxWeaponRange*1.5 then
                        platoon.dest=RUtils.LerpyRotate(platoon.Pos,target:GetPosition(),{targetDist,6+math.random(-2,5)})
                        self:Stop()
                        self:MoveToLocation(platoon.dest, false)
                        for _,v in platoonUnits do
                            if not v or v.Dead then continue end
                            ...
                            if v.Sniper or v:GetHealthPercent()<0.5 then
                                local upos=v:GetPosition()
                                local tdist=VDist2(targetPosition[1],targetPosition[3],upos[1],upos[3])
                                local smartPos = RUtils.lerpy({upos[1]+math.random(-2,2),upos[2],upos[3]+math.random(-2,2)},targetPosition,{tdist,tdist - v.MaxWeaponRange*v:GetHealthPercent()})
                                smartPos = {smartPos[1]+math.random(-1,1),smartPos[2],smartPos[3]+math.random(-1,1)}
                                local strafeshift=RUtils.LerpyRotate(upos,smartPos,{4,math.random(-4,4)})
                                IssueClearCommands({v})
                                IssueMove({v},strafeshift)
                                WaitTicks(1)
                            end
    ...
    HighlightTruePlatoon = function(self)
        ...
        while not platoon.dead and PlatoonExists(aiBrain, self) do
                ...
                if platoon.path and (platoon.navigating or platoon.pathretreat) then
                    ...
                    if pathline then
                        for i,node in pathline do
                            if i==1 then
                                DrawLinePop(platoon.Pos,node,pathcolor)
                            elseif i<=table.getn(pathline) then
                                DrawLinePop(pathline[i-1],node,pathcolor)
                            end
                ...
                for _,v in platoonUnits do
                    ...
                    for x = 1, v:GetWeaponCount() do
                        ...
                        if table.getn(instakills)>0 then
                            table.sort(instakills,function(a,b) return VDist3Sq(platoon.Pos,a:GetPosition())/math.pow(a:GetHealth(),2)<VDist3Sq(platoon.Pos,b:GetPosition())/math.pow(b:GetHealth(),2) end)
    ...
    PathNavigationRNG = function(self)
        ...
        while not platoon.dead and PlatoonExists(aiBrain, self) do
            ...
            if not platoon.path or VDist3Sq(platoon.path[table.getn(platoon.path)],platoon.Pos)<platoon.MaxWeaponRange*platoon.MaxWeaponRange then platoon.pathretreat=nil platoon.navigating=nil WaitTicks(20) continue end
            if platoon.navigating then
                ...
                if VDist3Sq(v:GetPosition(),platoon.Pos)>v.MaxWeaponRange/3*v.MaxWeaponRange/3+platoonNum then
                    if v.dest then
                        ...
                        if v.Sniper then
                            ...
                            IssueMove({v},RUtils.lerpy(platoon.Pos,platoon.dest,{VDist3(platoon.dest,platoon.Pos),v.MaxWeaponRange/6+math.sqrt(platoonNum)}))
                        end
                        spread=spread+VDist3Sq(v:GetPosition(),platoon.Pos)/v.MaxWeaponRange/v.MaxWeaponRange
                        snum=snum+1
                    else
            ...
            if VDist3Sq(path[1],platoon.Pos)<platoon.MaxWeaponRange*platoon.MaxWeaponRange*2 then
                if path[2] then
                    platoon.dest={path[1][1]+math.random(-4,4),path[1][2],path[1][3]+math.random(-4,4)}
                    self:MoveToLocation(platoon.dest,false)
                else
                    platoon.dest={path[1][1]+math.random(-4,4),path[1][2],path[1][3]+math.random(-4,4)}
                    self:MoveToLocation(platoon.dest,false)
    ...
    RemoveSingleUnitRNG = function(self, aiBrain, unit)
        -- Removes 1 uniy from a platoon without disbanding it.
